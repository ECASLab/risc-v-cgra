...
always @* begin
    if (op != 0) storedOP = op;
    case (op)
        7'b0010011: tempimmvalue = sign_extend(storedimm12);
        7'b0110111: tempimmvalue = {storedimmhi, 12'b000000000000};
        7'b1100011: tempimmvalue = sign_extend(storedimm12);
        7'b1101111: tempimmvalue = (storedimmhi[19] == 0) ? {12'b0, storedimmhi} : {12'b111111111111, storedimmhi};
        default: tempimmvalue = 32'b0;
    endcase
end
...
begin
    ...
    begin
        ...
        if (immvalue != 0 && state == 3'b0) //After acknowledge signal has been received
        begin
            state <= 3'b001;
            rdOut <= storedrd; 
            rdWrite <= 1; //Send signal to write output value into rd register
