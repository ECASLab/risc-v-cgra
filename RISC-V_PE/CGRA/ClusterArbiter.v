module cluster_mem_arbiter #(parameter NUM_CLUSTERS = 4)(
    ...
    if (reset) begin
        ...
        if (req_read != 0) $display("GlobalArbiter: Request status to read is: %b", req_read);
        if (req_write != 0) $display("GlobalArbiter: Request status to write is: %b", req_write);
        $display("Current use of bus is %d for current %b", working, current);
        ...
        if (!bus_locked) begin
            ...
            if (working[bus_owner] === 1'b0) begin
                $display("Bus owner %d is done working", bus_owner);
                if (release_wait) begin
                    $display("Bus is being released");
                    bus_locked <= 0;
                    release_wait <= 0;
                    grant_read      <= 4'b0000;
                    grant_write      <= 4'b0000;
        ...
        if ((grant_pending_read != 0 || grant_pending_write != 0) && !bus_locked) begin
            $display("Grant pending are: read %b and write %b", grant_pending_read, grant_pending_write);
